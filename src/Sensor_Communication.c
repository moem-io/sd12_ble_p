...
#define NRF_LOG_MODULE_NAME "[SNSR]"
...
uint32_t Send_Packet_Polling(char mode, uint8_t sensor, uint8_t id, char *data) {
    uint32_t err_code = Packet_Ok;

    err_code = makePacket(mode, sensor, id, data);
...
void printPacket(Packet *packet) {
    LOG_I("Packet.length : %d\r\n", packet->length);

    switch (packet->mode) {
        case Request:
            LOG_I("Packet.mode : Request\r\n");
            break;
        case Command:
            LOG_I("Packet.mode : Command\r\n");
            break;
        case Response:
            LOG_I("Packet.mode : Reponse\r\n");
            break;
        case Ack:
            LOG_I("Packet.mode : Ack\r\n");
            break;
        case Set_Address:
            LOG_I("Packet.mode : Set_Address\r\n");
            break;
        default:
            LOG_I("Packet.mode : None\r\n");
    }

    switch (packet->sensor) {
        case Th:
            LOG_I("Packet.sensor : Th\r\n");
            break;
        case Pressure:
            LOG_I("Packet.sensor : Pressure\r\n");
            break;
        case Light:
            LOG_I("Packet.sensor : Light\r\n");
            break;
        case Button:
            LOG_I("Packet.sensor : Button\r\n");
            break;
        case Human:
            LOG_I("Packet.sensor : Human\r\n");
            break;
        case Sound:
            LOG_I("Packet.sensor : Sound\r\n");
            break;
        case Rgb:
            LOG_I("Packet.sensor : Rgb\r\n");
            break;
        case Ir:
            LOG_I("Packet.sensor : Ir\r\n");
            break;
        case Buzzer:
            LOG_I("Packet.sensor : Buzzer\r\n");
            break;
        default:
            LOG_I("Packet.sensor : None\r\n");
    }

    LOG_I("Packet.id : %d\r\n", packet->id);

    LOG_I("Packet.data : %s\r\n", LOG_PUSH(packet->data));
}
